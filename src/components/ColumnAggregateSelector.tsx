...
interface ColumnAggregateSelectorProps {
  ...
  onAggregateChange: (columnId: string, aggregateType: AggregateType | null) => void;
...
export const ColumnAggregateSelector = ({
  ...
  const currentConfig = currentAggregate ? aggregateConfigs[currentAggregate] : null;

  return (
    <div className="flex flex-col items-center">
      {hasAggregate && currentConfig && (
        <div className={`text-xs px-2 py-1 rounded-full ${currentConfig.bgColor} ${currentConfig.color} font-medium mb-1 flex items-center gap-1 group/aggregate`}>
          <span>{currentConfig.label}: {formatValue(aggregateValue)}</span>
          <Button
            variant="ghost"
            size="sm"
        ...
        <PopoverContent side="top" align="center" className="w-64 p-0">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">📊 {columnLabel} Aggregates</CardTitle>
            </CardHeader>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${config.bgColor.replace('bg-', 'bg-').replace('-100', '-500')}`} />
